...
const log = {
    ...
    step: (current, total, msg) => console.log(`${colors.blue(`[${current}/${total}]`)} ${msg}`)
};

// Top-level folders (ignore scripts)
const TOP_LEVEL_FOLDERS = ['skills', 'rules', 'commands', 'agents'];
const IGNORE_DIRS = new Set(['scripts', 'node_modules', '.git']);

// Get package root (where ai/ folder is)
...
function validatePath(p) {
    ...
    if (!TOP_LEVEL_FOLDERS.includes(top)) {
        throw new Error(`Invalid path: ${p}. Must start with: ${TOP_LEVEL_FOLDERS.join(', ')}`);
    }
...
/**
 * Copy directory recursively (merge mode)
 */
...
async function install(options) {
    ...
    const destDir = path.join(process.cwd(), target === 'claude' ? '.claude' : '.cursor');

    // Parse --what
    let items;
    if (what) {
        items = what.split(',').map((s) => s.trim()).filter(Boolean);
    } else {
    ...
    log.step(2, 3, `Copying to ${target === 'claude' ? '.claude' : '.cursor'}/ (merge mode)...`);
    ...
    // Summary
    console.log('');
    log.success(`AI skills installed to ${target === 'claude' ? '.claude' : '.cursor'}/`);
    console.log('');
